...
export default function GradesPage() {
  ...
  const current = allGrades[selectedIdx]

  return (
    <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950">
      <Sidebar />
      <div className="flex-1 flex flex-col overflow-hidden">
        <Header title="ผลการเรียน" subtitle="ประวัติเกรดทุกภาคการศึกษา" />
                      ...
                      {current.courses.map((c) => (
                        <tr
                          ...
                          <td className="px-4 py-3 text-center">
                            <span
                              className={
                                GRADE_COLOR[c.grade] ?? "text-slate-600"
                              }>
                              {c.grade || "—"}
                            </span>
                          </td>
  ...
  return (
    <div
      className={`rounded-xl border px-5 py-4 shadow-sm ${
        highlight
          ? "bg-[#AC3520] border-[#AC3520] text-white"
