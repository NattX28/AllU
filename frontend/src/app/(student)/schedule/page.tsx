...
// ─── Day-based Palette with Gradient Shades ─────────────────
const DAY_PALETTES: Record<
  Day,
  { grad: string; glow: string; glass: string; border: string; shine: string }[]
> = {
  MON: [
    // เหลือง (หลายเฉดถ้ามีหลายวิชา)
    {
...
function buildGrid(timetable: TimetableResponse): Grid {
  ...
  // สำหรับจำว่าแต่ละวันมีกี่วิชาแล้ว (เอาไว้ไล่เฉดสี)
  ...
  timetable.courses.forEach((course) => {
    course.schedules.forEach((sch) => {
      ...
      // ถ้าวิชานี้ยังไม่เคยลงในวันนี้ ให้เพิ่มลงไปใน Map เพื่อรับ index เฉดสี
...
function SlotCard({ entry, span }: { entry: SlotEntry; span: number }) {
  const dayPalettes = DAY_PALETTES[entry.day]
  const p = dayPalettes[entry.colorIdx % dayPalettes.length]
  const wide = span >= 2
            ...
            <div
              ...
              <div className="flex flex-wrap gap-2">
                {timetable?.courses.map((course) => {
                  // หากล่องสีด้วยวันแรกที่เรียนของวิชานั้น เพื่อให้สีตรงกับตารางมากที่สุด
                  const firstSch = course.schedules[0]
                  const fallbackDay: Day = "MON"
                  const day = firstSch ? (firstSch.day as Day) : fallbackDay
                  const p = DAY_PALETTES[day][0]
